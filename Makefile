...
HEADERS = include/civetweb/civetweb.h
BUILD_RESOURCES =
...
ifdef WITH_CPP
  ...
  HEADERS += include/civetweb/CivetServer.h
  LCC = $(CXX)
else
  LCC = $(CC)
endif
...
install-slib: lib$(CPROG).so
	$(eval version=$(shell grep -w "define CIVETWEB_VERSION" include/civetweb/civetweb.h | sed 's|.*VERSION "\(.*\)"|\1|g'))
