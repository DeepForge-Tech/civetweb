...
but all functions required to run a HTTP server.
  ...
  - Optional: C++ wrapper
    - include/civetweb/CivetServer.h (C++ interface)
...
Note: The C++ wrapper uses the official C interface (civetweb/civetweb.h) without adding any features to the server itself. 
...
### C
  - Include the C interface ```civetweb/civetweb.h```.
...
### C++
  - Note that CivetWeb is Clean C, and C++ interface ```civetweb/CivetServer.h``` is only a wrapper layer around the C interface.
...
##### callbacks
Pointers to callback functions are passed to `mg_start` as well. They are documented in [civetweb/civetweb.h](https://github.com/civetweb/civetweb/blob/master/include/civetweb/civetweb.h) and the callbacks [API documentation](https://github.com/civetweb/civetweb/blob/master/docs/api/mg_callbacks.md).
